...
import { Search, Filter, Eye, Edit, Truck, Package, CheckCircle, Clock, X } from 'lucide-react'
...
const OrdersList = () => {
  ...
  const columns = [
    ...
    { key: 'items', label: 'Products', render: (value) => `${value?.length || 0} items` },
    ...
    { key: 'date', label: 'Date' },
    {
      ...
      render: (_, row) => (
        <div className="action-buttons">
          ...
          <button className="action-btn edit" title="Edit Order" onClick={() => handleEditOrder(row)}>
            <Edit size={16} />
          </button>
        </div>
      )
  ...
  return (
    <div className="orders-list">
      ...
      <div className="status-tabs">
        <button className={statusFilter === 'all' ? 'tab active' : 'tab'} onClick={() => setStatusFilter('all')}>All</button>
        <button className={statusFilter === 'pending' ? 'tab active' : 'tab'} onClick={() => setStatusFilter('pending')}>Pending</button>
        <button className={statusFilter === 'processing' ? 'tab active' : 'tab'} onClick={() => setStatusFilter('processing')}>Processing</button>
        <button className={statusFilter === 'shipped' ? 'tab active' : 'tab'} onClick={() => setStatusFilter('shipped')}>Shipped</button>
        <button className={statusFilter === 'delivered' ? 'tab active' : 'tab'} onClick={() => setStatusFilter('delivered')}>Delivered</button>
        <button className={statusFilter === 'cancelled' ? 'tab active' : 'tab'} onClick={() => setStatusFilter('cancelled')}>Cancelled</button>
      </div>
